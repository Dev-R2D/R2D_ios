...
public final class ActiveRideCoordinator {
    ...
    public func searchRoutes(origin: Coordinate, destination: Coordinate) async throws {
        state.routes = try await routes.searchRoute(origin: origin, destination: destination); state.lastRiskLayerUpdate = clock.now(); publish()
    }
    ...
    @discardableResult public func prepare() throws -> RideSession {
        ...
        let boss = BossContext(bossId: "boss-hanriver", snapshotVersion: "boss-v1", maxHp: 100)
        var session = try sessions.create(routeId: route.routeId, boss: boss)
        try RideStateMachine.transition(&session, to: .ready, at: clock.now()); try sessions.save(session)
        state.session = session
        state.game = .init(bossId: boss.bossId, bossSnapshotVersion: boss.snapshotVersion, maxHp: boss.maxHp, remainingHp: boss.maxHp, estimatedBaseDamage: 0, confirmedBaseDamage: 0, estimatedDataDamage: 0, confirmedDataDamage: 0, skillGauge: 0, connectionState: .online, processingState: .idle)
    ...
    @discardableResult public func start() throws -> RideSession {
        ...
        do { try location.start(sessionId: session.id); try sensors.start(sessionId: session.id, profileId: session.deviceProfileId) }
    ...
    public func sync() async {
        ...
        do { let snapshot = try await progressWorker.sync(rideId: session.id); apply(snapshot); state.connectionState = .online; state.game?.connectionState = .online }
        catch { state.connectionState = .offline; state.game?.connectionState = .offline; publish() }
    ...
    private func receive(location value: LocationSnapshot) {
        ...
        state.session = session
        if var game = state.game {
            game.estimatedBaseDamage = Int(session.localDistanceM / config.distanceStepM) * config.baseDamagePerStep
            game.skillGauge = min(1, session.localDistanceM / 1000); game.processingState = .awaitingServer; state.game = game
        }
    ...
    private func receive(chunk: SensorChunk) {
        ...
        if chunk.sampleCount > 30 { state.game?.estimatedDataDamage += 2; state.game?.processingState = .awaitingServer }
        try? sessions.save(session); publish()
