...
public final class CoreLocationTracker: NSObject, LocationTracker, CLLocationManagerDelegate, @unchecked Sendable {
    ...
    public func readiness() -> LocationReadiness {
        .init(servicesEnabled: true, authorization: normalize(manager.authorizationStatus), isAccuracySufficient: latestSample.map { $0.horizontalAccuracyM <= filter.configuration.maximumHorizontalAccuracyM } ?? false)
    }
    ...
    public func locationManagerDidChangeAuthorization(_ manager: CLLocationManager) {
        ...
        case .denied, .restricted:
            lock.withLock { running = false; paused = false }
        default:
            break
        }
    }
