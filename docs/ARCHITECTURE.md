...
`R2D.xcodeproj`의 R2D iOS 17 App Target은 `App/R2DApp.swift`만 소유하고, 로컬 Package의 `R2DAppSupport`와 `R2DUIKit`을 연결한다. `AppContainer.production()`이 CoreLocation/CoreMotion을 생성하며 Preview/Test는 Mock을 주입한다.

UIKit 화면과 `ActiveRideCoordinator`는 `@MainActor`다. CoreLocation delegate와 CoreMotion operation queue는 adapter 내부 lock으로 직렬화되며, 고빈도 raw sample은 UI로 전달되지 않는다. Coordinator는 저빈도 LocationSnapshot과 5초 SensorChunk만 받는다.
