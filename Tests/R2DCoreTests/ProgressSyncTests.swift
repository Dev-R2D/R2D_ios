...
@Test @MainActor func uploadAckTriggersAuthoritativeProgressFetch() async throws {
    let repository = ProgressRepositorySpy(), (coordinator, pipeline) = try await progressFixture(repository: repository)
    let before = await repository.count()
    ...
    #expect(await repository.count() == before + 1)
    #expect(coordinator.getSnapshot().rideProgress?.validDistance == 120)
}
