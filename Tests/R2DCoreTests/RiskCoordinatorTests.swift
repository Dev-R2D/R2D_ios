...
@Test @MainActor func locationUpdateEvaluatesWarningAndKeepsSafetyState() async throws {
    ...
    coordinator.switchView(.navigator); #expect(coordinator.getSnapshot().roadWarning?.cellID == "server-cell"); #expect(coordinator.getSnapshot().session?.state == .active)
}
